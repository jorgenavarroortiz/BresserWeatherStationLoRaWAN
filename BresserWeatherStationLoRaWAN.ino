...
TO DO:
  ...
  - Connect GPIOXXX to RST so that the whole system is reset in every reboot. If not, the LoRa chip is sometimes not initialized causing the system to fail.
...
#elif NODE==2
// deveui, little-endian
static const std::uint8_t deveui[] = { 0xC9, 0xB7, 0x05, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 };

// appeui, little-endian
static const std::uint8_t appeui[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

// appkey: just a string of bytes, sometimes referred to as "big endian".
static const std::uint8_t appkey[] = { 0xD0, 0x0D, 0xF3, 0xC3, 0xE2, 0x3C, 0x29, 0x30, 0xD2, 0xF2, 0x96, 0x6A, 0xBA, 0x89, 0x66, 0x27 };
#endif
...
RTC_NOINIT_ATTR uint32_t                        magicFlag3;               //!< flag for validating ESP32 boot using GPIO connected to RST
...
// Boot time, WDT reset time and reset reason
std::int32_t tBoot;
...
//        yield();
    ...
    resetReason = (int) rtc_get_reset_reason(0);
    ...
    log_d("LoRaWAN state will be cleared after %d reboots", noRebootsToEraseLoRaWANState);
    log_d("Boot count:        %d", bootCount);

    // LoRa pins
    log_v("PIN_LMIC_NSS:  %d", PIN_LMIC_NSS);
    log_v("PIN_LMIC_RST:  %d", PIN_LMIC_RST);
    log_v("PIN_LMIC_DIO0: %d", PIN_LMIC_DIO0);
    log_v("PIN_LMIC_DIO1: %d", PIN_LMIC_DIO1);
    log_v("PIN_LMIC_DIO2: %d", PIN_LMIC_DIO2);

    // After a certain number of reboots, the LoRaWAN state is reset just in case
    if ((noRebootsToEraseLoRaWANState != 0) && ((bootCount % noRebootsToEraseLoRaWANState) == 0)) {
      log_d("Erasing LoRaWAN state after %d reboots (current boot %d)", noRebootsToEraseLoRaWANState, bootCount);
      magicFlag1 = 0;
      magicFlag2 = 0;
    }
...
void OLEDShowInformation(void) {
    display.clear();
    display.drawString(0, 0, String(WSVERSION) + " #" + String(NODE));
    ...
    if ((lastRSSI < epsilon) && (lastRSSI > (-1*epsilon))) {
      display.drawString(0, 20, String(resetReason) + ", RSSI -");
    } else {
      display.drawString(0, 20, String(resetReason) + ", RSSI " + String(lastRSSI));
    }
    display.display();
}
#endif
